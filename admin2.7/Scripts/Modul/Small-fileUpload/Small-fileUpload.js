...
(function ($) {
    ...
    smallFileUpload.prototype.defaults = {
        ...
        ReferenceId:"",
        maxSizeInKb: 1024 * 4,
        allowExtension: ".doc, .docx, .xls, .xlsx, .ppt, .pptx, .jpg, .jpeg, .png, .gif, .zip, .pdf, .svg,.ico",
        privewType: "html",
        url: "/FileBase",
        multiple: false,
        uploadImmediately: true,
        deleteUrl: "/UploadFileBase/DeleteFiles3",
    ...
    smallFileUpload.prototype.doUpload = function () {
        ...
        if (valid) {
            ...
            options.success = function (result) {                
                ...
                //    $(this).parent().remove();
                //    instance.delete($(this).siblings(".f-link").attr("href"));
                //});
    ...
    smallFileUpload.prototype.delete = function (filePath) {
        var instance = this;
        var options = new window.FormData();
        options.url = instance.settings.deleteUrl;
        options.type = 'POST';
        options.data = { "filePath": filePath };
        options.success = function (result) {
            if (instance.uploadedFile != null) {
                for (var i = 0; i < instance.uploadedFile.length; i++) {
                    if (instance.uploadedFile[i].url.toLowerCase() == filePath.toLowerCase()) {
