...
namespace StoreMng
{
    ...
    class DatabaseAccess : IDisposable
    {
        ...
        {
            // conn = new SqlConnection("Data Source=localhost;Initial Catalog=storedata;Persist Security Info=True;User ID=storedata;Password=lanlep!$)%201411432");
        ...
        public DataTable executeSelect(string storeName, SqlParameter[] paramList, out int outPut)
        {
            DataSet dataSet = new DataSet();
            //Open connection
            conn = OpenConnection();
            try
            {
                adapter = new SqlDataAdapter(storeName, conn);
                adapter.SelectCommand.CommandType = CommandType.StoredProcedure;
                adapter.SelectCommand.Parameters.AddRange(paramList);
                adapter.SelectCommand.Parameters.Add("@output", SqlDbType.Int).Direction = ParameterDirection.Output;


                adapter.Fill(dataSet);
                outPut = Convert.ToInt32(adapter.SelectCommand.Parameters["@output"].Value);
            }
            catch (Exception)
            {
